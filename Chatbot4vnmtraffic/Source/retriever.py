...
class Retriever:
    def __init__(self, corpus, corpus_emb_path, model_name):
        ...
        if isinstance(data, list):
            self.embeddings = np.array([x["embedding"] for x in data])
        ...
        self.tokenized_corpus = [
            split_text(tokenize(doc.get("context", "")))
            for doc in self.corpus
        ]
        self.bm25 = BM25Okapi(self.tokenized_corpus)
    ...
    def retrieve(self, question, topk=10, final_k=3, threshold=0.5):
        # ===== Encode query =====
        segmented_q = tokenize(question)
        query_emb = self.embedder.encode([segmented_q])[0]
        ...
        # ===== BM25 scores =====
        tokenized_query = split_text(segmented_q)
        bm25_scores = self.bm25.get_scores(tokenized_query)
        ...
        for i, score in enumerate(combined_scores):
            if score >= threshold:
                candidates.append({
                    "id": self.corpus[i].get("id"),
                    "title": self.corpus[i].get("title", ""),
                    "context": self.corpus[i].get("context", ""),
                    "score": float(score)
                })

        # ===== Fallback =====
        if len(candidates) == 0:
            top_idx = np.argsort(combined_scores)[::-1][:topk]
            candidates = [
                {
                    "id": self.corpus[i].get("id"),
                    "title": self.corpus[i].get("title", ""),
        ...
        # ===== Reranking =====
        pairs = [(question, c["context"]) for c in candidates]
